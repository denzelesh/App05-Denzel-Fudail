...
play_game_lost_audio = GameLostAudioPlayer(lose_music_path)
play_game_won_audio = GameWonAudioPlayer(winning_music_path)
play_life_lost_audio = LifeLostAudioPlayer(player_life_lost_path)
...
regular_block_x = random.randint(-(game_screen_size / 2),
                                 (game_screen_size / 2))  # division by two as it calculates position..
...
lucky_block_x = random.randint(-(game_screen_size / 2),
                               (game_screen_size / 2))  # division by two as it calculates position..
...
game_metrics.write(starting_game_metric, align=game_metric_fontalignment, font=(
game_metric_fontname, game_metric_fontsize, game_metric_fonttype))  # The format of game metrics
start_lives = 0
start_score = 0
...
while gameplay == True:
    ...
    if impact_zone > mamba_head.distance(regular_block):
        ...
        game_metrics.write(variable_game_metric.format(players_score, remaining_lives), align=game_metric_fontalignment,
                           font=(game_metric_fontname, game_metric_fontsize,
                                 game_metric_fonttype))  # updates with new game metrics
    ...
    if lucky_blocks_collected == 5:
        game_metrics.clear()  # removes the previous metrics on screen
        game_metrics.write(game_won_message, align=game_metric_fontalignment,
                           font=(game_metric_fontname, game_metric_fontsize, game_metric_fonttype))  # Updates with
        # new game metrics
        #os.system("afplay " + winning_music_path)  # plays a sound effect & pauses the game
        play_game_won_audio.play_sound()
    ...
    # ...Division by two as it calculates position from the center of the screen
    if mamba_head.xcor() > (game_screen_size / 2) or mamba_head.xcor() < -(
            game_screen_size / 2) or mamba_head.ycor() > (game_screen_size / 2) or mamba_head.ycor() < -(
            game_screen_size / 2):
        ...
        play_life_lost_audio.play_sound()
        ...
        game_metrics.write(variable_game_metric.format(players_score, remaining_lives), align=game_metric_fontalignment,
                           font=(game_metric_fontname, game_metric_fontsize,
                                 game_metric_fonttype))  # updates with new game metric information
    ...
    for mamba_extenstion in mamba_extenstions:
        ...
        game_metrics.write(variable_game_metric.format(players_score, remaining_lives), align=game_metric_fontalignment,
                           font=(game_metric_fontname, game_metric_fontsize, game_metric_fonttype))

    if game_lost == 5:  # could have used boolean, but prior test show error causing incrorrect running of loop
        game_metrics.clear()
        game_metrics.write(game_lost_message, align=game_metric_fontalignment,
                           font=(game_metric_fontname, game_metric_fontsize, game_metric_fonttype))
        #os.system("afplay " + lose_music_path)  # plays lost game sound effect
        play_game_lost_audio.play_sound()
